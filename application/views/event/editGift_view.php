...
<div class="banner">
    <h2>
        ...
        <a href="javascript:void(0)" class="bread" rel="<?php echo $event_id; ?>" id="gift_view">Gift</a>
        <i class="fa fa-angle-right"></i>
        <span>Edit Gift</span>
    </h2>
</div>
...
<div class="blank">
    <div class="grid-form1">
        <h3 id="forms-horizontal">Edit Gift</h3>
        <?php
        $attributes = array('class' => 'form-horizontal', 'id' => 'edit_gift');
        echo form_open('event/edit_gift/'.$gift_id, $attributes);
        foreach($gift_detail as $gdt){ ?>
            <div id="the-message"></div>
            <div class="form-group">
                <label for="giftname" class="col-sm-2 control-label">Gift Name</label>
                <div class="col-sm-8">
                    <input type="text" name="giftname" value="<?php echo $gdt->gift_name ?>" class="form-control1" id="giftname" placeholder="Gift Name">
                </div>
            </div>
<!--            <div class="form-group">-->
...
<div class="modal fade" id="deleteGift" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
...
<script>
    $(document).ready(function() {
                    ...
                    if(response.success == true){
                        ...
                        $('.alert-success').delay(500).show(10, function() {
                            $(this).delay(3000).hide(10, function() {
                                $(this).remove();
                                var postData = {
                                    'view_name' : 'editGift_view',
                                    'item_id' : '<?php echo $gift_id ?>'
                                };
                                getContentView(postData);
                            });
                        })
        ...
        $('.modal-footer').on("click", ".delete_gift", function () {
            var view_name = 'gift_view';
            ...
            $.ajax({
                type:"POST",
                url: "<?php echo base_url('event/delete_gift/'.$gift_id)?>",
                //data:"id="+view_name,
                data:postValue,
                dataType: "json",
                success: function(response) {
